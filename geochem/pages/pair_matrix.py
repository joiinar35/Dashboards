...
pair_matrix_layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            # En la sección del sidebar, actualiza el texto de ayuda:
            ...
            dcc.Dropdown(
                ...
                multi=True,
                placeholder="Select upto 6 elements for pair matrix",
                disabled=df.empty
            ),
            dbc.Button(
           ...
           # En la sección del sidebar, actualiza el texto de ayuda:
            html.Div([
                html.P("Note: For performance and clarity, maximum 6 elements allowed for Pair Matrix.", 
                      className="small mt-3 text-warning"),  # Puedes usar text-warning para resaltar
                html.P("If you select more than 6, only the first 6 will be used.", 
                      className="small mt-3 text-warning")
            ]),
    ...
    for i, y_col in enumerate(elements):
        for j, x_col in enumerate(elements):
            ...
            if i == j:
                ...
                if len(hist_data) > 1:
                    try:
                        # Create distplot using figure_factory
                        ...
                        # Extract traces from distplot and add to our main figure
                        for trace in distplot_fig.data:
                            if 'histogram' in str(type(trace)).lower():
                                # This is the histogram trace
                                fig.add_trace(
                                    go.Histogram(
                                        x=hist_data,
                                        nbinsx=20,
                        ...
                        # Fallback to simple histogram if distplot fails
                        print(f"Distplot failed: {e}")
                        fig.add_trace(
                            go.Histogram(
                                x=hist_data,
                                nbinsx=20,
...
def pair_matrix_callbacks(app):
    ...
    @app.callback(
        Output('pair-matrix-element-selector', 'value'),
        [Input('pair-matrix-element-selector', 'value')],
        [State('pair-matrix-element-selector', 'options')]
    )
    def limit_element_selection(selected_values, available_options):
        """Limit selection to maximum 6 elements and show warning."""
        if selected_values and len(selected_values) > 6:
            print(f"Warning: Limited to 6 elements. Using first 6: {selected_values[:6]}")
            return selected_values[:6]
    ...
    def update_pair_matrix(n_clicks, sample_size, selected_elements):
        ...
        if n_clicks == 0 or not selected_elements:
            return go.Figure().update_layout(
                title=dict(
                    text="Please select upto 6 elements and click 'Generate Pair Matrix'",
                    x=0.5, y=0.5,
...
def register_pair_matrix_callbacks(app):
    """Register callbacks for lazy loading support."""
    return pair_matrix_callbacks(app)