...
def render_magnetic_gradients():
    """Render the magnetic gradients page"""
    st.markdown('<h1 class="main-header">Magnetic Gradients</h1>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.metric("Average Horizontal Gradient", f"{dBh_avg*1e3:.3f} ×10⁻³ nT/m")
        st.metric("Max Horizontal Gradient", f"{dBh_max*1e3:.3f} ×10⁻³ nT/m")
        st.metric("Average Vertical Gradient", f"{dBz_avg:.3f} nT/m")
        st.metric("Max Vertical Gradient", f"{dBz_max:.3f} nT/m")
    ...
    with col2:
        ...
        objects or the proximity to building remains, fences, etc.
    ...
    fig_horizontal.add_trace(
        go.Contour(
            ...
            y=yi[:,0],
            colorscale='Viridis',
            colorbar=dict(title='dBh (×10⁻³ nT/m)'),
    ...
    fig_horizontal.add_trace(
        go.Scatter(
            ...
            marker=dict(size=3, color='rgba(255, 255, 255, 0.7)', symbol='circle', line=dict(width=1, color='black')),
            showlegend=False,
